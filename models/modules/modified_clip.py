...
#         return x
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, drop_path=0., causal_mask=False):
        super().__init__()
...
"""
mask_type:  CLIPVisionTransformer: no mask
            CLIPTextEncoder: causal mask
            CLIPTextContextEncoder: causal mask
"""
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, drop_path_rate=0.,
        ...
        if mask_type in ["CLIPTextEncoder", "CLIPTextContextEncoder"]:
...
class TransformerDecoderLayer_scaled(nn.Module):
    def __init__(
        self,
        d_model,
        nhead,
        ...
        self.gamma_sa = nn.Parameter(torch.ones((1, 1, d_model)) * 1e-1)
        self.gamma_ca = nn.Parameter(torch.ones((1, 1, d_model)) * 1e-1)
        self.gamma_mlp = nn.Parameter(torch.ones((1, 1, d_model)) * 1e-1)

    def forward(self, x, mem):
        q = k = v = self.norm1(x)
        x = x + self.gamma_sa * self.self_attn(q, k, v)
        q = self.norm2(x)
        x = x + self.gamma_ca * self.cross_attn(q, mem, mem)
        x = x + self.gamma_mlp * self.dropout(self.mlp(self.norm3(x)))
        return x
...
class ContextDecoder_Hierachical(nn.Module):
    def __init__(self,
                 transformer_width=256,
                 transformer_heads=4,
                 transformer_layers=6,
        ...
        if if_scale:
            self.decoder = nn.ModuleList([
                        TransformerDecoderLayer_scaled(transformer_width, transformer_heads, dropout, if_flash=if_flash) for _ in range(transformer_layers)
                    ])
        else:
            self.decoder = nn.ModuleList([
                        TransformerDecoderLayer(transformer_width, transformer_heads, dropout, if_flash=if_flash) for _ in range(transformer_layers)
                    ])
...
class ContextDecoder_Hierachical_Agregate(nn.Module):
    def __init__(self,
                 transformer_width=256,
                 transformer_heads=4,
                 transformer_layers=6,
                 visual_dim=[512, 256, 128, 64],
                 text_dim=512,
                 dropout=0.1,
                 outdim=512,
                 if_scale=True,
        ...
        self.memory_proj_0 = nn.Sequential(
            nn.LayerNorm(visual_dim[0]),
            nn.Linear(visual_dim[0], transformer_width),
            nn.LayerNorm(transformer_width),
        )
        self.memory_proj_1 = nn.Sequential(
            nn.LayerNorm(visual_dim[1]),
            nn.Linear(visual_dim[1], transformer_width),
            nn.LayerNorm(transformer_width),
        )
        self.memory_proj_2 = nn.Sequential(
            nn.LayerNorm(visual_dim[2]),
            nn.Linear(visual_dim[2], transformer_width),
            nn.LayerNorm(transformer_width),
        )
        self.memory_proj_3 = nn.Sequential(
            nn.LayerNorm(visual_dim[3]),
            nn.Linear(visual_dim[3], transformer_width),
            nn.LayerNorm(transformer_width),
        )
    ...
    #     # B, N, C = visual.shape
    #     kv = []
    #     kv.append(self.memory_proj_0(visual[0]))
    #     kv.append(self.memory_proj_1(visual[1]))
    #     kv.append(self.memory_proj_2(visual[2]))
    #     kv.append(self.memory_proj_3(visual[3]))
    #     x = self.text_proj(text)

    #     for layer in self.decoder:
    #         x = sum([layer(x, kv[i]) for i in range(4)]) / 4.0
