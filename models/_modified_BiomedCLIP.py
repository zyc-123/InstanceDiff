...
from .BiomedCLIP.BiomedCLIP.hf_configs import arch_dict
from .BiomedCLIP.BiomedCLIP.hf_model import ClsPooler, _POOLERS
    ...
    def attention(self, x: torch.Tensor):
        if self.if_flash:
            ...
            return flash_attn_func(
                x.to(torch.float16).permute(1, 0, 2).reshape((B, N, self.n_head, self.d_model // self.n_head)),
                x.to(torch.float16).permute(1, 0, 2).reshape((B, N, self.n_head, self.d_model // self.n_head)),
                x.to(torch.float16).permute(1, 0, 2).reshape((B, N, self.n_head, self.d_model // self.n_head)),
                causal=self.causal_mask).to(torch.float32).reshape((B, N, self.d_model)).permute(1, 0, 2)
        else:
            self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
            return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]
...
class Attention_flash(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        # attn = attn.softmax(dim=-1)
        # x = torch.einsum('bknm,bmkc->bnkc', attn, v).reshape(B, N, C)
        x = flash_attn_func(torch.clamp(q, min=-255, max=255).to(torch.float16),
                            torch.clamp(k, min=-255, max=255).to(torch.float16),
                            torch.clamp(v, min=-255, max=255).to(torch.float16),
                            dropout_p=self.attn_drop,
                            softmax_scale=self.scale).reshape(B, N, C).to(torch.float32)
...
class TransformerDecoderLayer_scaled(nn.Module):
    def __init__(
        ...
        super().__init__()
        if if_flash:
            self.self_attn = Attention_flash(d_model, nhead, proj_drop=dropout)
            self.cross_attn = Attention_flash(d_model, nhead, proj_drop=dropout)
        else:
            self.self_attn = Attention(d_model, nhead, proj_drop=dropout)
            self.cross_attn = Attention(d_model, nhead, proj_drop=dropout)
...
class HFContextTextEncoder(nn.Module):
    """HuggingFace model adapter"""
    output_tokens: torch.jit.Final[bool]

    def __init__(
            self,
            model_name_or_path: str = 'microsoft/BiomedNLP-BiomedBERT-base-uncased-abstract',
            output_dim: int = 512,
        ...
        config_dict = {
            'architectures': ['BertForMaskedLM'], 'model_type': 'bert', 'attention_probs_dropout_prob': 0.1,
            ...
            'num_hidden_layers': 12, 'type_vocab_size': 2, 'vocab_size': 30522,
            '_commit_hash': 'd673b8835373c6fa116d6d8006b33d48734e305d'}
        unused_kwargs = {'name_or_path': 'microsoft/BiomedNLP-BiomedBERT-base-uncased-abstract'}
        config_class = CONFIG_MAPPING[config_dict["model_type"]]
        self.config = config_class.from_dict(config_dict, **unused_kwargs)
        self.transformer = modified_BertModel(self.config, add_pooling_layer=uses_transformer_pooler)
    ...
    def init_weights(self, pretrain_path):
        ...
        k, v = self.load_state_dict(new_state_dict, strict=False)
        print(f"{k}, {v}are misaligned params in text encoder")

    def token_embedding(self, input_ids, context):
        ...
        context = context.reshape(B, 1, N2, C).expand(B, K, N2, C)
        embeddings = torch.cat([inputs_embeds[:, :, 0:1], context, inputs_embeds[:, :, 1:]], dim=2).reshape(B*K, N1+N2, C)
        return embeddings

    def forward(self, x: TensorType, context: TensorType):
        # x: tokenize text
        # B, N1, C = x.shape
        K, N2, C = context.shape

        if self.pooler.cls_token_position != 0:
            self.pooler.cls_token_position += N2

        attn_mask = torch.ones((5, N2+x.shape[1])).long().to(x.device)
        mask_t = (x != self.config.pad_token_id).long()
        attn_mask[:, 0:1] = mask_t[:, 0:1]
        attn_mask[:, N2+1:] = mask_t[:, 1:]
        # attn_mask = (x != self.config.pad_token_id).long()
        # attn_mask = torch.cat([attn_mask[: 0:1].reshape((-1,1)), torch.zeros((5, 32)).long().to(x.device), attn_mask[: 1:]], dim=1)
        # out = self.transformer(input_ids=x, context=context, attention_mask=attn_mask) # B*K, N1+N2, 768
        out = self.transformer(inputs_embeds=self.token_embedding(x, context),
                               context=context,
                               attention_mask=attn_mask)

        pooled_out = self.pooler(out, attn_mask) # B*K, 768; choose cls token, seq idx: 0; K==1
        projected = self.proj(pooled_out) # B*K, 512; K==1
...
from transformers.models.bert.modeling_bert import BertModel, BertEmbeddings
from transformers.models.auto.configuration_auto import CONFIG_MAPPING
from typing import List, Optional

class modified_BertEmbeddings(BertEmbeddings):
    ...
    def forward(
        ...
        # K, N1, C = embeddings.shape
        # B, N2, C = context.shape
        # embeddings = embeddings.reshape(1, K, N1, C).expand(B, K, N1, C)
        # context = context.reshape(B, 1, N2, C).expand(B, K, N2, C)


        # embeddings = torch.cat([embeddings[:, :, 0:1], context, embeddings[:, :, 1:]], dim=2).reshape(B*K, N1+N2, C)
        extend_seq_length = seq_length # N1 + N2
        if position_ids is None:
            # position_ids = self.position_ids[:, past_key_values_length: seq_length + past_key_values_length]
            position_ids = self.position_ids[:, past_key_values_length: extend_seq_length + past_key_values_length]
...
class modified_BertModel(BertModel):
    def __init__(self, config, add_pooling_layer=True):
        ...
        # self.embeddings = modified_BertEmbeddings(config)
...
class ContextDecoder_Hierachical(nn.Module):
    def __init__(self,
        ...
        if if_scale:
            self.decoder = nn.ModuleList([
                TransformerDecoderLayer_scaled(transformer_width, transformer_heads, dropout, if_flash=if_flash) for _
                in range(transformer_layers)
            ])
        else:
            self.decoder = nn.ModuleList([
                TransformerDecoderLayer(transformer_width, transformer_heads, dropout, if_flash=if_flash) for _ in
